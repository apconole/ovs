...
/* See conntrack.h for the full API description. */
ct_private_id_t
...
conn_update_state_hook_register(int priority, conn_update_state_hook_fn fn)
{
    ...
    /* Insert in sorted order (ascending priority value = higher precedence). */
...
/* ALG helper dispatch table; populated by conntrack_ftp_init() and
 * conntrack_tftp_init() before any connections are processed. */
alg_helper alg_helpers[CT_ALG_CTL_SIP + 1];
 ...
 * false if the caller should fall through to the default conn_update_state(). */
static bool
