 ...
 *
 * Written once during single-threaded module initialisation (registration is
 * not thread-safe), then read-only during packet processing, so no additional
 * locking is needed.  Entries are kept sorted by ascending priority so
 * conn_update_state_dist() can iterate in order without extra bookkeeping.
 */
...
static bool ftp_conn_update_state_hook(struct conntrack *ct, struct dp_packet *,
                                       struct conn_lookup_ctx *, struct conn *conn,
                                       const struct nat_action_info_t *,
                                       enum ct_alg_ctl_type, long long,
                                       bool *)
    OVS_EXCLUDED(ct->ct_lock, ct->resources_lock, conn->lock);
