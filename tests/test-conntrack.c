...
test_ftp_alg_large_payload(struct ovs_cmdl_context *ctx OVS_UNUSED)
{
    ...
    dp_packet_batch_init(&syn_batch);
    dp_packet_batch_add(&syn_batch, syn);
    ...
    /* Build the a large payload: PORT command followed by padding spaces
    ...
    dp_packet_batch_init(&port_batch);
    dp_packet_batch_add(&port_batch, port_pkt);
